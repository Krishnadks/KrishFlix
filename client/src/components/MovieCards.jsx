...
const MovieCard = () => {
  ...
  const NewData = data;
  const Movie = NewData.map((d) => {
    let id = nanoid();
    return (
      <div
        key={id}
        className="flex flex-col justify-between z-5 text-white  py-4 px-3 bg-gray-800 rounded-2xl hover:-translate-y-1 transition duration-300 w-66"
      >
        <img
